...
export default function DebitMemoComponent() {
  ...
  const onCreateDebitAdvice = () => {
    supplierList({}, (selected: Supplier) => {
      if (selected) {
        debitMemo(
          { supplier: selected, type: "DEBIT_ADVICE" },
          (result: any) => {
            if (result?.success) {
  ...
  const onEditDebitAdvice = (record?: DebitMemo) => {
    debitMemo(
      { record: record, supplier: record?.supplier, type: "DEBIT_ADVICE" },
      (result: any) => {
        if (result?.success) {
          message.success(result?.message);
          refetch();
  ...
  return (
    <PageContainer content="Harmonizing Entries: Debit Advice Handling in Accounts Payable">
      <ProCard
        title="Debit Advice"
        headStyle={{
          flexWrap: "wrap",
          ...
          <Form layout="vertical" className="filter-form">
            <Row gutter={[16, 16]}>
              <Col xs={24} sm={12} md={8}>
                <FormDateRange
                  label="Filter Debit Advice Date"
        ...
        <DebitMemoTable
          ...
          handleOpen={(record) => onEditDebitAdvice(record)}
