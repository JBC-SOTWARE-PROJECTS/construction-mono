...
import {
  GET_PCV_AUTO_ENTRIES,
  POST_PETTY_CASH_PAYABLE,
  POST_ACCOUNT_PETTYCASH_MANUAL,
...
export default function PCVJournalEntries(props: IProps) {
  ...
  // ===============================
  const { loading } = useQuery<Query>(GET_PCV_AUTO_ENTRIES, {
    ...
    onCompleted: (data) => {
      let result = data?.pettyCashAccountView as JournalEntryViewDto[];
      if (!_.isEmpty(result)) {
        setLedger(result);
      } else {
        setManual(true);
  ...
  // ====================== Mutation ===============================
  const [postPettyCashPayable, { loading: upsertLoading }] =
    useMutation<Mutation>(POST_PETTY_CASH_PAYABLE, {
      ignoreResults: false,
      onCompleted: (data) => {
        const result = data.postPettyCash as PettyCash;
        if (result?.id) {
          hide(result);
        }
      },
  ...
  const [postPettyCashPayableManual, { loading: manualLoading }] =
    useMutation<Mutation>(POST_ACCOUNT_PETTYCASH_MANUAL, {
      ignoreResults: false,
      onCompleted: (data) => {
        const result = data.postPettyCashManual;
        if (result?.success) {
          hide(result?.message);
        }
      },
    ...
    <Modal
      title={
        <Typography.Title level={4}>
          <Space align="center">
            <ReconciliationOutlined /> Petty Cash Voucher Journal Entries
          </Space>
        </Typography.Title>
      }
      destroyOnClose={true}
