...
import {
  GET_DM_AUTO_ENTRIES,
  POST_DEBITMEMO_PAYABLE,
  POST_ACCOUNT_DEBITMEMO_MANUAL,
...
export default function RPJournalEntries(props: IProps) {
  ...
  // ===============================
  const { loading } = useQuery<Query>(GET_DM_AUTO_ENTRIES, {
    ...
    onCompleted: (data) => {
      let result = data?.dmAccountView as JournalEntryViewDto[];
      if (!_.isEmpty(result)) {
        setLedger(result);
      } else {
        setManual(true);
  ...
  // ====================== Mutation ===============================
  const [postDebitMemoPayable, { loading: upsertLoading }] =
    useMutation<Mutation>(POST_DEBITMEMO_PAYABLE, {
      ignoreResults: false,
      onCompleted: (data) => {
        const result = data.postDM as Reapplication;
        if (result?.id) {
          hide(result);
        }
      },
  ...
  const [postDebitMemoPayableManual, { loading: manualLoading }] =
    useMutation<Mutation>(POST_ACCOUNT_DEBITMEMO_MANUAL, {
      ignoreResults: false,
      onCompleted: (data) => {
        const result = data.postDManual;
        if (result?.success) {
          hide(result?.message);
        }
      },
    ...
    <Modal
      title={
        <Typography.Title level={4}>
          <Space align="center">
            <ReconciliationOutlined /> Debit Memo Journal Entries
          </Space>
        </Typography.Title>
      }
      destroyOnClose={true}
