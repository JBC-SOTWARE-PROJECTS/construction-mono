...
import { REGISTERED_FIXED_ASSET_PAGEABLE } from "@/graphql/assets/queries";
...
export default function FixedAssetSelector(props: IProps) {
  ...
  const { data, loading } = useQuery<Query>(REGISTERED_FIXED_ASSET_PAGEABLE, {
    ...
    <Modal
      title={
        <Typography.Title level={4}>
          <Space align="center">
            <ShoppingCartOutlined /> Fixed Asset List
          </Space>
        </Typography.Title>
      }
      destroyOnClose={true}
      ...
      <Row gutter={[8, 8]}>
        ...
        <Col span={24}>
          <Table
            ...
            loading={loading}
            columns={columns}
            dataSource={_.get(data, "page.content", []) as Item[]}
            pagination={false}
            footer={() => (
              <Pagination
                ...
                responsive={true}
                total={
                  _.get(data, "page.totalElements", 0) as number
                }
                onChange={(e) => {
