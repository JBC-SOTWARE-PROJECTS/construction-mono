...
@Service
@GraphQLApi
class LoanServices extends AbstractDaoCompanyService<Loan> {
...
//            row['paymentDate'] = localDate.plusMonths(i)
    ...
    Page<Loan> loanManagements(
        ...
        if(accountNo) {
            queryStr += "b.bankAccount.accountNumber in (:accountNo) and"
            countQueryStr += "b.bankAccount.accountNumber in (:accountNo) and"

            param.put("accountNo",accountNo)
        }

        queryStr += "(lower(b.bankAccount.bankname) like lower(concat('%',:filter,'%')) or b.loanNo like lower(concat('%',:filter,'%')) or b.referenceNo like lower(concat('%',:filter,'%'))) order by b.loanNo desc"
        countQueryStr += "(lower(b.bankAccount.bankname) like lower(concat('%',:filter,'%')) or b.loanNo like lower(concat('%',:filter,'%')) or b.referenceNo like lower(concat('%',:filter,'%')))"
