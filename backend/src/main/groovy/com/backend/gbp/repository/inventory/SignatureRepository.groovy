...
interface SignatureRepository extends JpaRepository<Signature, UUID> {
	...
	@Query(value = "Select s from Signature s where s.office.id = :departmentId and s.currentUsers = true and s.signatureType = :type")
	Signature findActiveSignatureDept(@Param("departmentId") UUID departmentId, @Param("type") String type)

	@Query(value = "Select s from Signature s where s.office.id = :departmentId and s.sequence = :sequence and s.signatureType = :type")
	...
	@Query(value = "Select count(s) from Signature s where s.office.id = :departmentId and s.signatureType = :type")
