...
interface EmployeeScheduleRepository extends JpaRepository<EmployeeSchedule, UUID> {
    ...
    @Query(value = """Select es from EmployeeSchedule es 
where es.dateString in :dates 
...
where es.dateString in :dates 
and es.employee.id = :employeeId
and es.isOvertime is true
 """)
    List<EmployeeSchedule> getOvertimeSchedules(@Param("dates") List<String> dates, @Param("employeeId") UUID employeeId)
