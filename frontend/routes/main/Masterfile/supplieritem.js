...
import { Card, Row, Col, Table, Button, Input, message, Modal, InputNumber } from 'antd';
import { useQuery, useMutation } from "@apollo/react-hooks";
import { gql } from "apollo-boost";
import { PlusCircleOutlined, DeleteFilled, ExclamationCircleOutlined } from '@ant-design/icons';
...
const SupplierItemContent = ({ account, id }) => {
    ...
    },
    {
        ...
        render: (text, record) => {
            return editable[record.id] ? (
                ...
                <span>{numeral(record.cost).format('0,0.00') + ' per ' + record?.item?.unit_of_usage?.unitDescription}</span>
            )
    ...
    return (
        <div>
            <Card title={`Supplier Items List of [${_.get(data, "sup.supplierCode")}] ${_.get(data, "sup.supplierFullname")}`} size="small"
                extra={
