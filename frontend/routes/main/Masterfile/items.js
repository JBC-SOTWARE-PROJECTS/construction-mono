...
const { Search } = Input;
const options = ["Edit", "Assign Item", "Assign Supplier"];
...
const ItemContent = ({ account }) => {
  ...
  const [modal, showModal] = dialogHook(ItemForm, (result) => {
    // item form
    if (result) {
  ...
  const [modalAssignItem, showModalAssignItem] = dialogHook(
    AssignItemForm,
    (result) => {
      // item form
      if (result) {
  ...
  const [modalAssignSupplierItem, showModalAssignSupplierItem] = dialogHook(
    AssignSupplierItemForm,
    (result) => {
      // item form
      if (result) {
        refetch();
