...
import PaymentTermForm from '../dialogs/paymentTermForm';
...
query($filter: String) {
    list: paymentTermList(filter: $filter) {
        id
        paymentCode
        paymentDesc
        paymentNoDays
...
const PaymentTerms = ({ account }) => {
    ...
    const [modal, showModal] = dialogHook(PaymentTermForm, (result) => {
        if (result) {
    ...
    {
        title: 'Payment Term Descriptions',
    ...
    return (
        <div className="pd-10">
            <Row>
                <Col {...colSearch}>
                    <Search placeholder="Search Payment Term"
