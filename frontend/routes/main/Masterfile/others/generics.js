...
import GenericForm from '../dialogs/genericForm';
...
query($filter: String) {
    list: genericList(filter: $filter) {
        id
        genericCode
        genericDescription
        isActive
...
const Generics = ({ account }) => {
    ...
    const [modal, showModal] = dialogHook(GenericForm, (result) => {
        if (result) {
    ...
    return (
        <div className="pd-10">
            <Row>
                <Col {...colSearch}>
                    <Search placeholder="Search Generics"
