...
import { postSTIObj } from "../../../shared/constant";
...
const POST_REC = gql`
  mutation ($items: [Map_String_ObjectScalar], $parentId: UUID) {
    upsert: postInventoryLedgerIssuance(items: $items, parentId: $parentId) {
      id
    }
  }
`;

const PostIssuance = ({ visible, hide, ...props }) => {
  ...
  useEffect(() => {
    if (_.get(data, "items")) {
      ...
      (data?.items || []).map(async (e, i) => {
        ...
        let ex = {};
        let sto = {};
        if (props?.issueType === "EXPENSE") {
          if (props?.issueFrom?.id === props?.issueTo?.id) {
            ex = postSTIObj("EX", e, props, i);
            payload.push(ex);
          } else {
            ex = postSTIObj("EX", e, props, i);
            sti = postSTIObj("STI", e, props, i);
            sto = postSTIObj("STO", e, props, i);
            payload.push(sto);
            payload.push(sti);
            payload.push(ex);
          }
