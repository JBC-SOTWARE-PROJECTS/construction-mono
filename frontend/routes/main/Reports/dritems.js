...
const DReportItemContent = ({ account }) => {
  ...
  const print = () => {
    window.open(
      `${getUrlPrefix()}/reports/inventory/print/srr_detail/${
        state.start + "T00:00:00Z"
      }/${state.end + "T23:00:00Z"}`
    );
  };
  ...
  const columns = [
    {
      ...
      render: (txt, record) => <span>{record?.receivingReport?.rrNo}</span>,
    },
    {
      ...
      render: (receiveDate, record) => (
        <span key={receiveDate}>
          {moment(record.receiveDate).format("YYYY-MM-DD")}
        </span>
      ),
    },
    {
    ...
    },
    {
      ...
      key: "item.descLong",
      render: (txt, record) => <span>{record?.item?.descLong}</span>,
    },
    {
      title: "Item Category",
    ...
    },
    {
      ...
      key: "receiveUnitCost",
      align: "right",
      render: (receiveUnitCost) => (
        <span>{numeral(receiveUnitCost).format("0,0.00")}</span>
      ),
    },
    {
      title: "Qty",
  ...
  return (
    <Card
      title="Delivery Receiving Report Items Sumarry"
      size="small"
      extra={
        <span>
          <Button
