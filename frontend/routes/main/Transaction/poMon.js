...
const POMonContent = ({ account }) => {
  ...
  const [modal, showModal] = dialogHook(ViewPOMonitoring, (result) => {
    // item form
    if (result) {
  ...
  const columns = [
    ...
    {
      title: (
        <ColTitlePopUp descripton="Order Qty (UoP)" popup="Unit of Purchase" />
      ),
      dataIndex: "quantity",
      key: "quantity",
      render: (quantity) => <span>{numeral(quantity).format("0,0")}</span>,
    },
    {
      title: (
        <ColTitlePopUp descripton="Order Qty (UoU)" popup="Unit of Usage" />
      ),
      dataIndex: "qtyInSmall",
      key: "qtyInSmall",
      render: (qtyInSmall) => <span>{numeral(qtyInSmall).format("0,0")}</span>,
    },
    {
      title: (
        <ColTitlePopUp descripton="Delivered Qty (UoU)" popup="Unit of Usage" />
      ),
    ...
    {
      title: (
        <ColTitlePopUp descripton="Balance Qty (UoU)" popup="Unit of Usage" />
      ),
  ...
  return (
    <Card title="Purchase Orde Delivery Monitoring" size="small">
      <Row>
        <Col {...col18}>
          <Search
            placeholder="Search Purchase Order Items"
