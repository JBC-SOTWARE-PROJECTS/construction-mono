...
import { Card, Row, Col, Table, Button, Input, InputNumber, message, Divider, Modal } from 'antd';
...
const MarkupContent = ({ account }) => {
    ...
    const [upsertPrice, { loading: upsertPriceLoading }] = useMutation(UPSERT_PRICES, {
        ignoreResults: false,
        onCompleted: (data) => {
            if (!_.isEmpty(data?.upsert?.id)) {
    ...
    const columns = [
        ...
        },
        {
            ...
            align: "right",
            render: (markup) => (
                <span>{numeral(markup).format('0,0.00') + "%"}</span>
            )
        },
        {
            title: <ColTitlePopUp descripton="Selling Price (w/ VAT)" editable={true} />,
            ...
            render: (text, record) => {
                return editable[record.id + "sellingPrice"] ? (
                    ...
                    <span key={text}>{numeral(record.sellingPrice).format('0,0.00')}</span>
                )
            }
        },
        {
            ...
            align: "right",
            render: (govPrice) => (
                <span>{numeral(govPrice).format('0,0.00')}</span>
            )
        }
    ];

    return (
        <Card title="Price and Markup Control" size="small"
            extra={
                <span>
                    <Button size="small" type="primary" icon={<CheckCircleOutlined />} className="margin-0"
                        onClick={onDefault} loading={upsertLoading}
