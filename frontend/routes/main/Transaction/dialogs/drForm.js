...
import { Col, Row, Button, Divider, Table, InputNumber, Form, message, Alert, Checkbox, DatePicker, Skeleton, Modal } from "antd";
import { DeleteFilled, BarcodeOutlined, ExclamationCircleOutlined } from '@ant-design/icons';
...
import { col4, col8, createObjRecPo, decimal } from "../../../../shared/constant";
...
const UPSERT_RECORD = gql`
 mutation($id: UUID, $items: [Map_String_ObjectScalar], $fields: Map_String_ObjectScalar) {
...
const DRForm = ({ visible, hide, ...props }) => {
    ...
    const { loading: loadingPoItems, fetchMore: fetchPoItems } = useQuery(GET_PR_ITEMS, {
        variables: {
            id: null,
        },
    });
    ...
    const [modal, showModal] = dialogHook(InventoryBySupplierModal, async (result) => { // item form
    ...
    const recItems = () => {
        ...
        if (supplier) { //show items with supplier
            showModal({ show: true, myProps: { items: items, type: "REC", supplier: supplier } })
        } else { //show all items that is belong to this office
    ...
    const vatConfigCalculation = async (vatInc, vat) => {
        ...
        if (!_.isEmpty(items)) {
            ...
            _.forEach(items, function (obj, key) {
                if (vatInc) {
                    inputTax = ((obj.totalAmount - obj.receiveDiscountCost) / (vatRate + 1)) * vatRate;
    ...
    const setTotalSumarry = async (cost, inputTax, netAmount, discount, obj) => {
        const { getFieldValue, setFieldsValue } = form
        let gross = decimal(_.sum(cost)); let disc = decimal(_.sum(discount));
        let tax = decimal(_.sum(inputTax)); let net = decimal(_.sum(netAmount));
        ...
        setFieldsValue({ ['amount']: numeral(inc ? (gross - disc) : net).format('0,0.00') });
    ...
    const onChangeArray = async (el, obj, value) => {
        ...
        if (el === "receiveQty") {
            ...
            if (obj.refPoItem) {
                data = update(items, {
                    [key]: {
                        ...
                        isCompleted: {
                            $set: !_.isEmpty(obj.refPoItem) ? value >= obj.refPoItem?.deliveryBalance ? true : false : true,
                        },
                        isPartial: {
                            $set: !_.isEmpty(obj.refPoItem) ? value < obj.refPoItem?.deliveryBalance ? true : false : false,
                        },
                    },
                });
            } else {
            ...
            let inputTax = amount.vatInclusive
                ? ((obj.totalAmount - obj.receiveDiscountCost) / (vatRate + 1)) * vatRate
            ...
            return {
                onDoubleClick: () => {
                    if (props?.isPosted || props?.isVoid) {
                        message.error("This Delivery Receiving is already posted/voided. Editing is disabled.")
                    } else {
    ...
    },
    {
        ...
        render: (text, record) => {
            return editable[record.id + "receiveUnitCost"] ? (
                ...
                <span key={text}>{numeral(record.receiveUnitCost).format('0,0.00')}</span>
            )
    ...
    },
    {
        ...
        render: (text, record) => {
            return editable[record.id + "discountRate"] ? (
                ...
                <span key={text}>{numeral(record?.discountRate).format('0,0.00')}</span>
            )
    ...
    },
    {
            ...
            return {
                onDoubleClick: () => {
                    if (props?.isPosted || props?.isVoid) {
                        ...
                        setEditable({ ...editable, [e.id + "receiveDiscountCost"]: true })
                    }
                }, // double click row
            };
        },
        render: (text, record) => {
            return editable[record.id + "receiveDiscountCost"] ? (
                ...
                <span key={text}>{numeral(record?.receiveDiscountCost).format('0,0.00')}</span>
            )
    ...
    },
    {
        ...
        render: (text, record) => {
            return editable[record.id + "expirationDate"] ? (
                ...
                <span key={text}>{record?.expirationDate ? moment(record.expirationDate).format("YYYY-MM-DD") : 'N/A'}</span>
            )
        ...
        render: (text, record) => {
            return (
                <Checkbox
                    ...
                    onChange={(e) => onChangeArray("isPartial", record, e?.target?.checked)}
                    disabled={props?.isPosted || props?.isVoid}
                />
            )
        ...
        render: (text, record) => {
            return (
                <Checkbox
                    ...
                    onChange={(e) => onChangeArray("isCompleted", record, e?.target?.checked)}
                    disabled={props?.isPosted || props?.isVoid}
                />
            )
        ...
        await fetchPoItems({
            ...
            updateQuery: (prev, { fetchMoreResult }) => {
                    ...
                    (list || []).map((e) => {
                        ...
                        if (amount.vatInclusive) {
                            input_tax = e?.item?.vatable
                                ? (totalAmount / (amount.vatRate / 100 + 1)) * (amount.vatRate / 100)
                                : 0;
                            net_amount = totalAmount - input_tax;
                        } else {
                            input_tax = e.item.vatable ? totalAmount * (amount.vatRate / 100) : 0;
                            net_amount = totalAmount + input_tax;
                        }
                        //end calculation
                ...
                if (_.isEmpty(tobedeleted)) {
                    ...
                    recalCulateForexLocal(payload, { po: e, tobedeleted: tobedeleted, payload: payload })
                }
    ...
    //triggers
    useMemo(() => { //use memo to avoid memory leaks
        if (props?.id) {
    ...
    return (
        <CModal
            ...
            footer={[
                ...
                <Button form="poForm" key="submit" htmlType="submit" type="primary" loading={upsertLoading}
            ...
            {loading ? (<Skeleton active />) : (
                <>
                        ...
                        <Row>
                            <Col span={24}>
                                <Form.Item style={{ marginBottom: 0 }}>
                                    <label className="srr-label">SRR Number: <span>{props?.id ? props?.rrNo : "Auto Generated"}</span></label><br />
                                    <label className="srr-label-recby">Received By: <span>{props?.id ? props?.userFullname : account?.fullName}</span></label>
                                    <Alert message="Purchase Order No must appear on all related correspondence, shipping papers and invoices." type="info" showIcon />
                                </Form.Item>
                            </Col>
                            <Col span={4}>
                                <FormInput description={"Delivery Date"}
                            ...
                            <Col {...col4}>
                                <FormInput description={"Remarks/Notes"}
                            ...
                            {/*  */}
                            <Col {...col8}>
                                <FormInput description={"Gross Amount"}
                                    name="grossAmount"
                                    initialValue={numeral(amount.grossAmount).format('0,0.00')}
                                    placeholder="Gross Amount" disabled />
                            </Col>
                            <Col {...col8}>
                                <FormInput description={"Total Discount"}
                                    name="totalDiscount"
                                    initialValue={numeral(amount.totalDiscount).format('0,0.00')}
                                    placeholder="Total Discount" disabled />
                            </Col>
                            <Col {...col8}>
                                <FormInput description={"Net of Discount"}
                                    name="netDiscount"
                                    initialValue={numeral(amount.netDiscount).format('0,0.00')}
                                    placeholder="Net of Discount" disabled />
                            </Col>
                            <Col {...col8}>
                            ...
                            <Col {...col8}>
                                <FormCheckbox description={"Vat Inclusive ?"}
                                    name="vatInclusive"
                                    valuePropName="checked"
                                    horizontal={true}
                                    onChange={(e) => changeVatConfig("vatInclusive", e?.target?.checked)}
                                    initialValue={amount.vatInclusive}
                                    disabled={props?.isPosted || props?.isVoid}
                                    field="Vat Inclusive" />
                            </Col>
                            <Col {...col8}>
                                <FormInput description={"Input Tax"}
                                    name="inputTax"
                                    initialValue={numeral(amount.inputTax).format('0,0.00')}
                                    placeholder="Input Tax" disabled />
                            </Col>
                            <Col {...col8}>
                                <FormInput description={"Net of Vat"}
                                    name="netAmount"
                                    initialValue={numeral(amount.netAmount).format('0,0.00')}
                                    placeholder="Net of Vat" disabled />
                            </Col>
                            <Col {...col8}>
                                <FormInput description={"Total Amount"}
                                    name="amount"
                    ...
                    <Row>
                        <Col span={24}>
                            ...
                            <div className="float-right">
                                <Button icon={<BarcodeOutlined />} type="primary" size="small"
                                    onClick={recItems} disabled={props?.isPosted || props?.isVoid}
                                >
                                    Receive Items
                                </Button>
                            </div>
                        ...
                        <Col span={24}>
                            <Table
                                loading={loading || removeLoading || loadingPoItems || overrideLoading}
                                scroll={{ x: 3000 }}
                                columns={columns}
                                dataSource={items}
