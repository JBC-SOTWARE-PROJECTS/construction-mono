...
import { Col, Row, Button, Divider, Table, InputNumber, Form, message, Input } from "antd";
...
import { col4, col3, createObj, typeLabel, poType } from "../../../../shared/constant";
...
const UPSERT_RECORD = gql`
 mutation($id: UUID, $items: [Map_String_ObjectScalar], $forRemove: [Map_String_ObjectScalar], $fields: Map_String_ObjectScalar) {
     upsert: upsertPO(id: $id, items: $items, forRemove: $forRemove, fields: $fields) {
         id
	}
}`;
...
const POForm = ({ visible, hide, ...props }) => {
    ...
    const { loading: loadingPrItems, data: dataPrItems, fetchMore: fetchPrItems } = useQuery(GET_PR_ITEMS, {
        variables: {
            prNos: prNo,
    ...
    const orderItems = () => {
        ...
        if (supplier) { //show items with supplier
            showModal({ show: true, myProps: { items: items, type: "PO", supplier: supplier } })
            ...
            return {
                onDoubleClick: () => {
                    if (props?.isApprove || props?.isVoided) {
                        message.error("This Purchase Order is already approved/voided. Editing is disabled.")
                    } else {
    ...
    {
        title: <ColTitlePopUp descripton="Qty (UoP)" popup="Unit of Purchase" editable={true} />,
        dataIndex: 'quantity',
        key: 'quantity',
        onCell: (e) => {
            return {
    ...
    {
        title: <ColTitlePopUp descripton="Unit Cost (UoP)" popup="Unit of Purchase" editable={true} />,
        dataIndex: 'unitCost',
        key: 'unitCost',
        onCell: (e) => {
            return {
            ...
            <Table
                ...
                columns={[
                    ...
                    },
                    {
                        ...
                        render: (text, record) => {
                            return (
                                <span key={text}>{numeral(record.requestedQty).format('0,0')}</span>
                            );
                        },
                    },
    ...
    const mapObject = (list) => {
        ...
        if (!_.isEmpty(list)) {
            ...
            if (props?.id) { //removes items to database
                let tobeRemove = _.filter(payload, function (o) { //remove item where not included in PR items
                    return !_.includes(itemIds, o.item.id) && !_.isEmpty(o.prNos);
                });
                if (!_.isEmpty(tobeRemove)) { //remove query if naay sulod
                    setForRemove([...forRemove, ...tobeRemove])
                }
            }
            payload = _.filter(payload, function (o) { //remove item where not included in PR items
    ...
    return (
        ...
        >
            ...
            >
                <Row>
                    <Col {...col4}>
                        <FormInput description={"Purchase Order #"}
                            name="poNumber"
                            initialValue={props?.poNumber}
                            placeholder="Auto Generated" disabled />
                    </Col>
                    <Col {...col4}>
                        <FormInput description={"Purchase Order Date"}
                    ...
                    <Col {...col4}>
                        <FormInput description={"ETA Date"}
                            rules={[{ required: true, message: 'This Field is required' }]}
                    ...
                    <Col {...col3}>
                        <FormInput description={"Prepared By"}
                            name="preparedBy"
                ...
                <Col span={24}>
                    ...
                    <div className="float-right">
                        <Button icon={<BarcodeOutlined />} type="primary" size="small"
                            onClick={orderItems} disabled={props?.isApprove || props?.isVoided || !_.isEmpty(prNo)}
                        >
                            Order Items
                        </Button>
                    </div>
