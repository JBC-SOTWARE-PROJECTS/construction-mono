...
const RTSForm = ({ visible, hide, ...props }) => {
  ...
  const returnItems = () => {
    ...
    if (supplier) {
      ...
      showModal({
        show: true,
        myProps: { items: items, type: "RTS", supplier: supplier },
      });
    } else {
        ...
        return {
          onDoubleClick: () => {
            if (props?.isPosted || props?.isVoid) {
              message.error(
                "This Return Supplier is already posted/voided. Editing is disabled."
              );
            } else {
    ...
    },
    {
      ...
      render: (text, record) => {
        return editable[record.id + "returnUnitCost"] ? (
          ...
          <span key={text}>
            {numeral(record.returnUnitCost).format("0,0.00")}
          </span>
        );
      },
    },
