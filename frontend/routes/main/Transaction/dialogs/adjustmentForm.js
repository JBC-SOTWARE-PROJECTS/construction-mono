...
import { Col, Row, Button, Divider, Table, InputNumber, Form, message, Input, Skeleton, Dropdown, Menu, Tag, Modal } from "antd";
...
const AdjustmentForm = ({ visible, hide, ...props }) => {
    ...
    const columns = [
                ...
                return {
                    onDoubleClick: () => {
                        if (e?.isPosted || e?.isCancel) {
                            message.error("This Quantity Adjustment is already posted/voided. Editing is disabled.")
                        } else {
        ...
        },
        {
            ...
            key: 'unit_cost',
            render: (unit_cost) => (
                <span>{numeral(unit_cost).format('0,0.00')}</span>
            )
        },
        {
            title: 'Status',
                ...
                <>
                    ...
                    >
                        <Row>
                            ...
                            <Col {...col4}>
                                <FormInput description={"Quantity (+ -) (UoU)"}
                                    name="quantity"
                                    type="number"
                                    rules={[{ required: true, message: 'This Field is required' }]}
                    ...
                    <Row>
                        ...
                        <Col span={24}>
                            <Table
                                loading={loading || upsertLoading || upsertQtyLoading || postVoidLoading}
