...
import { Col, Row, Button, Divider, Table, InputNumber, Form, message } from "antd";
...
import { col3, PR_TYPE } from "../../../../shared/constant";
...
const PRForm = ({ visible, hide, ...props }) => {
    ...
    const [modalSuplier, showSupplierModal] = dialogHook(InventoryBySupplierModal, (result) => { // item form
        if (!_.isEmpty(result)) { // validate here
            if (_.isEmpty(items)) {
                setItems(result)
            } else { //append
    ...
    const requestItems = () => {
        ...
        if (supplier) { //show items with supplier
            showSupplierModal({ show: true, myProps: { items: items, type: "PR", supplier: supplier } })
    ...
    },
    {
        ...
        onCell: (e) => {
            return {
                onDoubleClick: () => {
                    if (props?.isApprove) {
                        message.error("This Purchase Request is already approved. Editing is disabled.")
                    } else {
    ...
    return (
        <CModal
            ...
            footer={[
                ...
                <Button form="prForm" key="submit" htmlType="submit" type="primary" loading={upsertLoading}
        ...
        >
            ...
            >
                <Row>
                    <Col {...col3}>
                        <FormInput description={"Purchase Request #"}
                            name="prNo"
                            initialValue={props?.prNo}
                            placeholder="Auto Generated" disabled />
                    </Col>
                    <Col {...col3}>
                        <FormInput description={"Date Needed"}
                    ...
                    <Col {...col3}>
                        <FormInput description={"Requested By"}
                            name="userFullname"
                ...
                <Col span={24}>
                    ...
                    <div className="float-right">
                        <Button icon={<BarcodeOutlined />} type="primary" size="small"
                            onClick={requestItems} disabled={props?.isApprove}
                        >
                            Request Items
                        </Button>
                    </div>
