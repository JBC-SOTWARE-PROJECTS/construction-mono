...
const POContent = ({ account }) => {
  ...
  const [modal, showModal] = dialogHook(DRForm, (result) => {
    // item form
    if (result) {
  ...
  const [modalPost, showPostModal] = dialogHook(PostReceiving, (result) => {
    // item form
    if (result) {
      message.success(result);
      refetch();
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <span key={text}>
          {moment(record?.receiveDate).format("MMM DD, YYYY")}
        </span>
      ),
    },
    {
  ...
  return (
    ...
    >
      <Row>
        <Col span={24}>
          <Search
            placeholder="Search Delivery Receiving"
