...
query($filter: String, $office: UUID, $page: Int, $size: Int ) {
    list: rtsByFiltersPage(filter: $filter, office: $office,page: $page, size: $size) {
        content{
            id
            rtsNo
            returnDate
...
const RTSContent = ({ account }) => {
    ...
    const [modal, showModal] = dialogHook(RTSForm, (result) => { // item form
        if (result) {
    ...
    const [modalPost, showPostModal] = dialogHook(PostSupplierReturns, (result) => { // item form
        if (result) {
            message.success(result);
            refetch()
        }
    ...
    const columns = [{
        ...
        key: 'returnDate',
        render: (text, record) => (
            <span key={text}>{moment(record?.returnDate).format("MMM DD, YYYY")}</span>
        )
    },
    {
        title: 'RTS #',
    ...
    return (
        <Card title="Return to Supplier List" size="small"
            extra={
                <span>
