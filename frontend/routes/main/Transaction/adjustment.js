...
import AdjustmentForm from './dialogs/adjustmentForm';
...
const ADJContent = ({ account }) => {
    ...
    const [modal, showModal] = dialogHook(AdjustmentForm, (result) => { // item form
        if (result) {
    ...
    const disabledMenu = (option, record) => {
        ...
        if (option === "View Transaction") {
            result = _.indexOf(account?.user?.access, 'allow_edit_qty_adjustment') <= -1
        }
        return result;
    }
    ...
    return (
        <Card title="Quantity Adjustment" size="small">
            <Row>
                <Col {...col18}>
