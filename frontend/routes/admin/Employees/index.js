...
query($filter: String, $status: Boolean, $office: UUID, $position: UUID) {
    list: employeeByFilter(filter: $filter, status: $status,office: $office,position: $position) {
        id
        employeeNo
        fullName
        position {
...
const Employee = ({ account }) => { //state first before useQuery
        ...
        return <div className="gx-module-side">
            <div className="gx-module-side-header">
                <div className="gx-module-logo">
                    ...
                    <span><IntlMessages id="sidebar.admin.employee" /></span>
                </div>
            </div>
                    ...
                    <div className="gx-module-side-nav">
                        <ul className="gx-module-nav">
                            {filterOptions.map(option => <li key={option.id} className="gx-nav-item">
                                <span
                                    className={`gx-link ${option.status === state.selectedStatus ? 'active' : ''}`} onClick={() =>
                        ...
                        <ul className="gx-module-nav">
                            <li key="all-offices" className="gx-nav-item">
                                <span
                                    className={`gx-link ${!state.selectedOffice && 'active'}`} onClick={() =>
                                        setState({ ...state, selectedOffice: null })
                                    }>
                                    <i className={`icon icon-all-contacts`} />
                                    <span>All Offices</span>
                                </span>
                            </li>
                            {(filterData?.office || []).map(option => <li key={option.id} className="gx-nav-item">
                                <span
                                    className={`gx-link ${option.id === state.selectedOffice ? 'active' : ''}`} onClick={() =>
                        ...
                        <ul className="gx-module-nav">
                            <li key="all-offices" className="gx-nav-item">
                                <span
                                    className={`gx-link ${!state.selectedPos && 'active'}`} onClick={() =>
                                        setState({ ...state, selectedPos: null })
                                    }>
                                    <i className={`icon icon-all-contacts`} />
                                    <span>All Offices</span>
                                </span>
                            </li>
                            {(filterData?.pos || []).map(option => <li key={option.id} className="gx-nav-item">
                                <span
                                    className={`gx-link ${option.id === state.selectedPos ? 'active' : ''}`} onClick={() =>
                        ...
                        <CustomScrollbars className="gx-module-content-scroll">
                            {loading ? (
                                ...
                                _.get(data, 'list', []).length === 0 ? (
                                    ...
                                    <EmployeeList emplist={_.get(data, 'list', [])}
