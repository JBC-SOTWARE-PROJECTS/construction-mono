...
const AddPaymentForm = ({ visible, hide, ...props }) => {
  ...
  const onSubmit = (value) => {
    ...
    if (
      value?.amount > 0 &&
      _.get(paymentData, "bill.balance") >= value?.amount
    ) {
      let obj = {
        amount: value?.amount,
        bill_id: props?.id,
      ...
      message.error(
        "Please Enter Amount to Pay Greater than Zero and Not Greater than the Balance"
      );
    }
  };
  ...
  const columns = [
    {
      ...
      render: (createdDate, record) => (
        <Text type={record.voided && "danger"} delete={record.voided && true}>
          {moment(createdDate).format("MM/DD/YYYY h:mm:ss A")}
        </Text>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (txt, record) => (
        <Button
          ...
          onClick={() =>
            window.open(
              `${getUrlPrefix()}/reports/billing/print/receipt/${record.id}`
            )
          }
          type="danger"
          size="small"
          ...
          {_.isEmpty(_.get(data, "activeShift")) ? (
            ...
            <>
              <Row>
                  ...
                  >
                    <Row>
                      <Col span={6}>
                        ...
                        <br />
                        <a style={{ fontSize: 30 }} onClick={clickBalance}>
                          {numeral(
                            _.get(paymentData, "bill.balance", 0)
                          ).format("0,0.00")}
                        </a>
                      </Col>
                      <Col span={12}>
                ...
                <Col span={24}>
                  <Card
                    title={`Payments History of ${
                      props?.customer?.fullName
                        ? props?.customer?.fullName
                        : props?.otcName
                    }`}
                    size="small"
                    extra={<a>{`Billing No: ${props?.billNo}`}</a>}
                  >
                    <Table
                      size="small"
                      columns={columns}
                ...
                <Col span={24}>
                  <Alert
                    message={`${_.get(
                      data,
                      "activeShift.terminal.terminal_no"
                    )} | ${_.get(data, "activeShift.shiftNo")} | ${moment(
                      new Date()
