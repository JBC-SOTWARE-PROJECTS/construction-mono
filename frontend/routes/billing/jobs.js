...
import EndorsementForm from "./dialogs/endorsementForm";
...
const JobOrders = ({ account }) => {
  ...
  const [endorsement, showEndorsement] = dialogHook(
    EndorsementForm,
    (result) => {
      // item form
      if (result) {
  ...
  const menus = (record) => (
    <Menu
      onClick={(e) => {
        ...
        } else if (e.key === "Endorsement Form") {
          showEndorsement({ show: true, myProps: record });
        }
      }}
    >
      {options.map((option) => (
